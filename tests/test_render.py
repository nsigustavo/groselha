...
class GrosaTestCase(TestCase):
    ...
    def test_should_subtitute_attribute_of_context(self):
        self.template("<b content='tweet.title'>title</b>", {'tweet': {'title': 'example'}}).equal("<b>example</b>")
    
    def test_should_subtitute_content(self):
        self.template("<b content='title'>Title</b>", {'title': 'example'}).equal("<b>example</b>")
    
    def test_not_filtered_toHtml_should_render_html_scaped(self):
        self.template("<div><p content='text_html' /></div>", {"text_html": "<b>text</b>"}).equal("<div><p>&lt;b&gt;text&lt;/b&gt;</p></div>")
    
    def test_filter_toHtml_should_render_no_scaped_text(self):
        self.template("<div><p content='text_html|toHtml'>teste</p></div>", {"text_html": "<b>texto com &aacute;</b>"}).equal("<div><p><b>texto com &aacute;</b></p></div>")
    
    def test_should_remove_element_if_var_not_in_context(self):
        self.template("<div><img condition='image' /></div>", {}).equal("<div></div>")
    
    def test_should_remove_condition_statement_if_var_in_context(self):
        self.template("<div><img condition='image' /></div>", {'image': 'test.png'}).equal("<div><img /></div>")
  
    def test_should_replace_the_tag_with_the_text_equivalent_to_the_value_in_the_dictionary(self):
        self.template("<div><a>Titulo:<span replace='title'>title</span></a></div>", {'title': 'example'}).equal("<div><a>Titulo:example</a></div>")
        self.template("<div><a><span replace='title'>title</span>:Titulo</a></div>", {'title': 'example'}).equal("<div><a>example:Titulo</a></div>")
    ...
    def test_should_repeat_this_tag_for_each_item_in_my_list_of_object_values(self):
        ...
        self.template("<ul><li repeat='tweet tweets' ><a content='tweet.text'>test</a></li></ul>", context
    ...
    def test_repeat_should_keep_ordering(self):
        context = {"items": [1, 2, 3]}
        self.template("<ul><li repeat='item items' content='item'></li></ul>", context).equal(
  ...
  """)
    ...
    def test_The_condition_statement_should_remove_element_if_a_function_in_context_return_false(self):
        ...
        self.template("<div><img condition='test'/></div>", context).equal("<div></div>")
    ...
    def test_The_condition_statement_not_should_remove_element_if_a_function_in_context_return_false(self):
        ...
        self.template("<div><img condition='test'/></div>", context).equal("<div><img /></div>")
    ...
    def test_The_content_statement_should_subtitute_content_of_tag_with_returned_of_function(self):
        ...
        self.template("<head><title content='title'/></head>", context).equal("<head><title>grosa is cool</title></head>")
    ...
    def test_The_replace_statement_should_subtitute_tag_with_returned_of_function(self):
        ...
        self.template("<div>title:<span replace='title'>title</span></div>", context).equal("<div>title:grosa is cool</div>")
  
    def test_should_format_string(self):
        template = self.template("<h1 content='string:hello {{name}}'></h1>", {'name':'Gustavo Rezende'})
        template.equal("<h1>hello Gustavo Rezende</h1>")
  
    def test_must_format_a_string_with_multiple_contexts(self):
        template = self.template("<h1 content='string:hello {{user.alias}}, {{user.name}}'></h1>", {'user':{'alias':'groselha','name':'Gustavo Rezende'}})
    ...
    def test_The_value_of_src_attribute_should_be_replace_with_value_of_context(self):
        ...
        self.template("<div>image:<img attr:src='photo.src' src='' /></div>", context).equal("<div>image:<img src='image.png' /></div>")
    ...
    def test_The_value_of_attributes_should_be_replace_with_values_of_context(self):
        ...
        self.template("<div>image:<img  attr:alt='text' attr:src='photo' /></div>", context).equal("<div>image:<img alt='alternative text' src='image.png' /></div>")
    
    def test_The_value_of_attributes_should_be_replaced_with_values_of_context_with_function_return(self):
        def photo():
            return 'image.png'
        context = {"photo": photo}
        self.template("<div>image:<img attr:src='photo' /></div>", context).equal("<div>image:<img src='image.png' /></div>")
    
    def test_the_condition_should_verify_values_of_context_with_function_returned(self):
        def photo():
            return 'image.png'
        context = {"photo": photo}
        self.template("<div>image:<img condition='photo' attr:src='photo' src='' /></div>", context).equal("<div>image:<img src='image.png' /></div>")
    
    def test_element_with_condition_should_not_render_if_function_in_context_returns_falsy(self):
        def photo():pass
        context = {"photo": photo}
        self.template("<div>image:<img condition='photo' attr:src='photo' src='' /></div>", context).equal("<div>image:</div>")
    
    # def test_should_render_templateTags(self):
    #     @Grosa.extendTemplateTags
    #     def hello(node):
    #         return "hello "+ node["name"]
    #     self.template("<h1><hello name='Gustavo Rezende'>guy</hello></h1>").equal("<h1>hello Gustavo Rezende</h1>")
